...
class Pokemon:
    ...
    async def info(self):
        ...
        if not self.name:
            ...
            self.ability = await self.ability_info()
            self.exp = await self.base_exp()
        return f"The name of your Pokémon: {self.name}\n Ability name: {self.ability}\n Base Experience: {self.exp}\n Health: {self.hp}\n Attack: {self.power}"  # Returning the string with the Pokémon's name
...
class Fighter(Pokemon):
    ...
    async def feed(self, feed_interval = 20, hp_increase = 17 ):
        current_time = datetime.now()  
        delta_time = timedelta(hours=feed_interval)  
        if (current_time - self.last_feed_time) > delta_time:
            self.hp += hp_increase
